...
static char *get_share_name(void) {
  ...
  if (name == NULL) {
    ...
    if (strcmp(host_name, "localhost") == 0) {
      ...
      name = g_strdup_printf(_("%s's public files on %s"), g_get_user_name(),
                             host_name);
    }
  }
  /* And truncate */
...
static void httpd_child_setup(gpointer user_data) {
#ifdef HAVE_SELINUX
  char *mycon;
  ...
  if (is_selinux_enabled()) {
    ...
    if (setexeccon(mycon) < 0) abort();
    freecon(mycon);
  }
#endif
}

static const char *known_httpd_locations[] = {HTTPD_PROGRAM, "/usr/sbin/httpd",
                                              "/usr/sbin/httpd2",
                                              "/usr/sbin/apache2", NULL};

static char *get_httpd_program(void) {
  int i;
...
static const char *known_httpd_modules_locations[] = {
    HTTPD_MODULES_PATH, "/etc/httpd/modules", "/usr/lib/apache2/modules", NULL};

static gchar *get_httpd_modules_path(void) {
  int i;
  ...
  for (i = 0; known_httpd_modules_locations[i]; i++) {
    ...
    if (g_file_test(known_httpd_modules_locations[i],
...
static char *get_httpd_config(const char *httpd_program) {
  ...
  if (!g_spawn_command_line_sync(cmd_line, &standard_output, NULL, NULL,
                                 NULL)) {
    g_free(cmd_line);
    return NULL;
  }
...
static gboolean spawn_httpd(int port, pid_t *pid_out) {
  ...
  free6 = env[i++] =
      g_strdup_printf("XDG_CONFIG_HOME=%s", g_get_user_config_dir());
  free7 = env[i++] = g_strdup_printf("GUS_SHARE_NAME=%s", get_share_name());
  free8 = env[i++] =
      g_strdup_printf("GUS_LOGIN_LABEL=%s", "Please log in as the user guest");
  free9 = env[i++] =
      g_strdup_printf("HTTP_MODULES_PATH=%s", get_httpd_modules_path());
  env[i++] = "LANG=C";
  env[i] = NULL;

  pid_filename =
      g_build_filename(g_get_user_config_dir(), "user-share", "pid", NULL);
  ...
  error = NULL;
  res = g_spawn_sync(g_get_home_dir(), argv, env, 0, httpd_child_setup, NULL,
...
void http_down(void) { kill_httpd(); }

gboolean http_init(void) { return TRUE; }

pid_t http_get_pid(void) { return httpd_pid; }
