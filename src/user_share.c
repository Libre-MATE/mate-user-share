...
static void obex_services_start(void) {
  GSettings *settings;

  if (bluetoothd_enabled == FALSE || has_console == FALSE) return;

  settings = g_settings_new(GSETTINGS_SCHEMA);

  if (g_settings_get_boolean(settings,
                             FILE_SHARING_BLUETOOTH_OBEXPUSH_ENABLED) == TRUE) {
    obexpush_up();
  }
...
static void obex_services_shutdown(void) {
  obexpush_down();
  obexftp_down();
}

static void session_properties_changed_cb(GDBusProxy *session,
                                          GVariant *changed, char **invalidated,
                                          gpointer user_data) {
  GVariant *v;

  v = g_variant_lookup_value(changed, "SessionIsActive",
                             G_VARIANT_TYPE_BOOLEAN);
  if (v) {
    has_console = g_variant_get_boolean(v);
    g_debug("Received session is active change: now %s",
            has_console ? "active" : "inactive");
...
static gboolean is_session_active(void) {
  GVariant *variant;
  gboolean is_session_active = FALSE;

  variant = g_dbus_proxy_get_cached_property(session_proxy, "SessionIsActive");
  if (variant) {
...
static void session_init(void) {
  ...
  session_proxy = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, MATE_SESSION_DBUS_NAME,
      MATE_SESSION_DBUS_OBJECT, MATE_SESSION_DBUS_INTERFACE, NULL, &error);
  ...
  g_signal_connect(session_proxy, "g-properties-changed",
                   G_CALLBACK(session_properties_changed_cb), NULL);
  has_console = is_session_active();
}

static void default_adapter_changed(GObject *gobject, GParamSpec *pspec,
                                    gpointer user_data) {
  char *adapter;
  gboolean adapter_powered;

  g_object_get(G_OBJECT(client), "default-adapter", &adapter,
               "default-adapter-powered", &adapter_powered, NULL);
...
static void migrate_old_configuration(void) {
  const char *old_config_dir;
  const char *new_config_dir;

  old_config_dir =
      g_build_filename(g_get_home_dir(), ".mate2", "user-share", NULL);
  new_config_dir =
      g_build_filename(g_get_user_config_dir(), "user-share", NULL);
  if (g_file_test(old_config_dir, G_FILE_TEST_IS_DIR))
    g_rename(old_config_dir, new_config_dir);
}

static void require_password_changed(GSettings *settings, gchar *key,
                                     gpointer data) {
...
static gboolean disabled_timeout_callback(gpointer user_data) {
  ...
  if (g_settings_get_boolean(settings, FILE_SHARING_BLUETOOTH_ENABLED) ==
    ...
    _exit(0);
#else  /* HAVE_BLUETOOTH */
  if (g_settings_get_boolean(settings, FILE_SHARING_ENABLED) == FALSE) _exit(0);
#endif /* HAVE_BLUETOOTH */
  return FALSE;
}
...
static void file_sharing_enabled_changed(GSettings *settings, gchar *key,
  ...
  if (enabled) {
    ...
    http_down();
    disabled_timeout_tag = g_timeout_add_seconds(
        3, (GSourceFunc)disabled_timeout_callback, settings);
  }
}
  ...
  if (g_settings_get_boolean(settings, FILE_SHARING_BLUETOOTH_ENABLED) ==
    ...
    if (g_settings_get_boolean(settings, FILE_SHARING_ENABLED) == FALSE &&
        g_settings_get_boolean(
            settings, FILE_SHARING_BLUETOOTH_OBEXPUSH_ENABLED) == FALSE) {
      _exit(0);
    }
...
static int x_io_error_handler(Display *xdisplay) {
  http_down();
#ifdef HAVE_BLUETOOTH
  obexftp_down();
#endif /* HAVE_BLUETOOTH */
...
int main(int argc, char **argv) {
  ...
  if (g_strcmp0(g_get_real_name(), "root") == 0) {
    g_warning(
        "mate-user-share cannot be started as root for security reasons.");
    return 1;
  }
  ...
  selection_owner = XCreateSimpleWindow(xdisplay, RootWindow(xdisplay, 0), 0, 0,
                                        1, 1, 0, 0, 0);
  ...
  if (!file_sharing_enabled()) return 1;
  ...
  if (http_init() == FALSE) return 1;
#ifdef HAVE_BLUETOOTH
  if (obexftp_init() == FALSE) return 1;
  if (obexpush_init() == FALSE) return 1;
#endif /* HAVE_BLUETOOTH */
