...
#define CAJA_USER_SHARE(o) \
  (G_TYPE_CHECK_INSTANCE_CAST((o), CAJA_TYPE_USER_SHARE, CajaUserShare))
#define CAJA_IS_USER_SHARE(o) \
  (G_TYPE_CHECK_INSTANCE_TYPE((o), CAJA_TYPE_USER_SHARE))
...
static void launch_process(char **argv, GtkWindow *parent) {
  GError *error;
  GtkWidget *dialog;
  ...
  if (!g_spawn_async(NULL, argv, NULL, 0, NULL, NULL, NULL, &error)) {
    dialog = gtk_message_dialog_new(
        ...
        _("Unable to launch the Personal File Sharing preferences"));
...
static void launch_prefs_on_window(GtkWindow *window) {
  char *argv[2];
...
static void bar_response_cb(CajaShareBar *bar, gint response, gpointer data) {
...
#ifdef HAVE_BLUETOOTH
static void downloads_bar_set_from_bluetooth_status(GtkWidget *bar) {
  BluetoothClient *client;
  gboolean bt_powered;

  client = g_object_get_data(G_OBJECT(bar), "bluetooth-client");
  g_object_get(G_OBJECT(client), "default-adapter-powered", &bt_powered, NULL);
  gtk_widget_set_visible(bar, bt_powered);
}

static void default_adapter_powered_cb(GObject *gobject, GParamSpec *pspec,
                                       GtkWidget *bar) {
  downloads_bar_set_from_bluetooth_status(bar);
}
#endif /* HAVE_BLUETOOTH */

static GtkWidget *caja_user_share_get_location_widget(
    CajaLocationWidgetProvider *iface, const char *uri, GtkWidget *window) {
  GFile *file;
  GtkWidget *bar;
  guint i;
  ...
  if (is_dir[0] != FALSE) {
    ...
    if (is_dir[1] != FALSE) {
      ...
      bar =
          caja_share_bar_new(_("May be shared over the network or Bluetooth"));
    }
#else
    ...
    g_object_set_data_full(G_OBJECT(bar), "bluetooth-client", client,
                           g_object_unref);
  ...
  g_signal_connect(bar, "response", G_CALLBACK(bar_response_cb), window);
...
static void caja_user_share_location_widget_provider_iface_init(
    CajaLocationWidgetProviderIface *iface) {
  iface->get_widget = caja_user_share_get_location_widget;
}

static void caja_user_share_instance_init(CajaUserShare *share) {}

static void caja_user_share_class_init(CajaUserShareClass *klass) {}

static GType share_type = 0;

static GType caja_user_share_get_type(void) { return share_type; }
...
static void caja_user_share_register_type(GTypeModule *module) {
  ...
  share_type = g_type_module_register_type(module, G_TYPE_OBJECT,
                                           "CajaUserShare", &info, 0);
